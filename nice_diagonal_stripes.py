...
#  Kernel_inhibitor = fft(kernel_inhibitor)
  ...
  kernel_inhibitor = zeros(Field.shape[0])
  kernel_inhibitor[0:inh] = 1.0 / inh
  kernel_diff = kernel_producer - kernel_inhibitor
  kernel_diff = fft(kernel_diff)
  res = real(ifft(Field * kernel_diff[:, newaxis], axis=0))
  res = real(ifft(fft(res,axis=1) * kernel_diff[newaxis, :], axis=1))
  return res
...
#  INH = array([8])
#  AMOUNT = array([0.01])
  fields = zeros((field.shape[0],field.shape[1],PROD.size))
